...
#include<stdio.h>
#include<stdlib.h>
//adding two matrices of dimension n
int** sumofmatrices(int n,int **a,int **b)
{
	//Dynamic allocation of aaray
...
int	**SquareMatrixMultiplications(int n,int a[][n], int b[][n])
{
	...
	if(n == 3)
	{//multiplying matrices of dimensios 2x2 using Strassen's algorithm for matrix multiplication
	...
	{// partioning matrices of dimensions more than 2x2
		...
		temp -= 1;
		//combining all 2x2 matrices to get nxn mtrices 
		for(i=1;i<=temp;i++)
		{
			for(j=1;j<=temp;j++)
				c[i][j] = s11[i][j];
...
int main()
{
	...
	//printing elements of product matrix
	for(i=1;i<=n;i++)
	{
		for(j=1;j<=n;j++)
			printf("%d ",p[i][j]);
